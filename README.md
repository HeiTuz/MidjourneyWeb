...
A further full Editor batch used Smart Select on the painted photographer. The selection expanded from a jacket click to the whole figure, so the applied erase mask and instruction targeted the whole person. Four candidates completed and one 1456×816 JPG was saved. The jacket changed to muted red and the window/camera arrangement broadly remained, but the rebuilt person lost the painted look. Selection, erase, generation and file delivery passed; style preservation did not.
